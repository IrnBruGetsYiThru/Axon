...
rgetfield r_lua_getfield = (rgetfield)unprotect(x(0xAAAAAA));

typedef char*(__stdcall* rtolstring)(DWORD rL, int idx, size_t *size);

rtolstring r_lua_tolstring = (rtolstring)(unprotect(x(0xAAAAAA)));
typedef void(__fastcall* rsettop)(DWORD rL, int idx);

rsettop r_lua_settop = (rsettop)(unprotect(x(0xAAAAAA)));
typedef bool(__cdecl* toboolean)(DWORD rL, bool idx);

toboolean r_lua_toboolean = (toboolean)(x(0xAAAAAA));
typedef void(__fastcall* pushvalue)(DWORD rL, DWORD idx);

pushvalue r_lua_pushvalue = (pushvalue)(unprotect(x(0xAAAAAA)));
typedef double(__thiscall* pushnumber)(DWORD rL, double idx);

pushnumber r_lua_pushnumber = (pushnumber)(unprotect(x(0xAAAAAA)));
typedef void(__fastcall* rpushstring)(DWORD rL, const char*);

rpushstring r_lua_pushstring = (rpushstring)(x(0xAAAAAA));
typedef int(__cdecl* pcall)(DWORD rL, DWORD, DWORD, DWORD);

pcall r_luad_pcall = (pcall)(unprotect(x(0xAAAAAA)));
typedef void(__cdecl* pushnil)(DWORD);

pushnil r_lua_pushnil = (pushnil)(unprotect(x(0xAAAAAA)));
...
rboolean r_lua_pushboolean = (rboolean)(unprotect(x(0xAAAAAA)));
...
rpushcclosure r_lua_pushcclosure = (rpushcclosure)(unprotect(x(0xAAAAAA)));
typedef void(__cdecl *rcreatetable)(DWORD rL, int num, int fix);

rcreatetable r_lua_createtable = (rcreatetable)(unprotect(x(0xAAAAAA)));
typedef void*(__cdecl *rnewuserdata)(DWORD rL, int idx);

rnewuserdata r_lua_newuserdata = (rnewuserdata)(unprotect(x(0xAAAAAA)));
typedef void*(__stdcall *rgetmetatable)(DWORD rL, int idx);

rgetmetatable r_lua_getmetatable = (rgetmetatable)(unprotect(x(0xAAAAAA)));
typedef void*(__cdecl *rsettable)(DWORD rL, int);

rsettable r_lua_settable = (rsettable)(unprotect(x(0xAAAAAA)));
typedef DWORD(__cdecl *rtype)(DWORD, int);

rtype r_lua_type = (rtype)(x(0xAAAAAA));
typedef DWORD(__cdecl *rnewthread)(DWORD);

rnewthread r_lua_newthread = (rnewthread)unprotect(x(0xAAAAAA));
typedef DWORD(__cdecl *rref)(DWORD, DWORD);


#include "retcheck.h"
rref r_luaL_ref = (rref)(Ret::unprotect<DWORD>((byte*)(x(0xAAAAAA))));

typedef void(__cdecl *rrawgeti)(DWORD, DWORD, DWORD);
rrawgeti r_lua_rawgeti = (rrawgeti)unprotect(x(0xAAAAAA));

typedef int(__cdecl *gettop)(DWORD);
gettop r_lua_gettop = (gettop)(x(0xAAAAAA));

typedef void(__cdecl *rpushlight)(DWORD, void*);
rpushlight r_lua_pushlightuserdata = (rpushlight)(unprotect(x(0xAAAAAA)));

typedef void*(__cdecl *touserdata)(DWORD, int);
touserdata r_lua_touserdata =(touserdata)unprotect(x(0xAAAAAA));

typedef void*(__cdecl *rindex2adr)(DWORD, int);
touserdata r_lua_index2adr = (rindex2adr)unprotect(x(0xAAAAAA));
